...
skewness_value = skewness(dw_dy_sampl_x06);
kurtosis_value = kurtosis(dw_dy_sampl_x06);
...
figure;
histogram(dw_dy_sampl_x06, 'Normalization', 'pdf', 'BinEdges', linspace(min(dw_dy_sampl_x06), max(dw_dy_sampl_x06), 30), 'EdgeColor', 'w');
title('Probability Density Function of Wall-Normal Velocity');
xlabel('Wall-Normal Velocity');
ylabel('Density');
grid on;

% Display skewness and kurtosis on the plot
text(0.7, 0.9, sprintf('Skewness: %.4f', skewness_value), 'Units', 'normalized', 'BackgroundColor', 'w');
text(0.7, 0.85, sprintf('Kurtosis: %.4f', kurtosis_value), 'Units', 'normalized', 'BackgroundColor', 'w');

saveas(gcf, 'Normal_Distrbution.png');
