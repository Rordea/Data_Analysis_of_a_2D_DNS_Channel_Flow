...
figure(12);
hold on;
plot(t_smpl, w_smpl_x06, ':b', 'LineWidth', 2);
plot(t_smpl, dw_dy_sampl_x06, ':r', 'LineWidth', 2);
...
% Display the plot
saveas(gcf, 'wallgrad_w_x0_06_sample.png');



% Assuming dt is the time step between samples
dt = t_smpl(2) - t_smpl(1);
Fs = 1 / dt; % Sampling frequency

% Perform Fourier transformation
N = length(dw_dy_sampl_x06);
...
figure(13);
subplot(2,1,1);
plot(t_smpl, dw_dy_sampl_x06, '-b', 'LineWidth', 2);
title('Time Domain Signal');
xlabel('Time [s]');
ylabel('dw/dx');

subplot(2,1,2);
plot(frequencies, amplitude_spectrum, '-r', 'LineWidth', 2);
title('Frequency Domain Signal');
xlabel('Frequency [Hz]');
ylabel('Amplitude');

% Display the plot
saveas(gcf, 'Fourier_Transformation.png');
