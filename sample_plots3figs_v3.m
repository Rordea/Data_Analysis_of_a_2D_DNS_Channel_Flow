...
hold on
plot(x_smpl,avg_u(1,:),'-r',LineWidth=2)
%plot(x_smpl,uplim_u(1,:),':r',LineWidth=1)

plot(x_smpl,avg_v(1,:),'-b',LineWidth=2)
%plot(x_smpl,uplim_v(1,:),':b',LineWidth=1)

plot(x_smpl,avg_w(1,:),'-k',LineWidth=2)
%plot(x_smpl,uplim_w(1,:),':k',LineWidth=1)

line([x_smpl(idx_max_stddev_w), x_smpl(idx_max_stddev_w)], [0, value_at_max_stddev_w], 'Color', 'k', 'LineStyle', ':', 'LineWidth', 1);



% plot(x_smpl,lwlim_u(1,:),':r',LineWidth=1)
% plot(x_smpl,lwlim_v(1,:),':b',LineWidth=1)
% plot(x_smpl,lwlim_w(1,:),':k',LineWidth=1)


xlabel('$x/\delta$','Interpreter','latex','FontSize',12,'FontName','Times New Roman')
%xlim([0,1])
legend('$u_{s_{avg}}$','$u_{s_{+/-std}}$','$v_{s_{avg}}$','$v_{s_{+/-std}}$','$w_{s_{avg}}$','$w_{s_{+/-std}}$','$std_{max}$',...
    'Interpreter','latex','FontSize',12,'Location','east','FontName','Times New Roman')
%xticks(0:0.1:1);
%yticks(-0.2:0.25:1.5);
grid on
set(gca,'TickLabelInterpreter','latex','FontSize',12)
saveas(gcf, 'Average_wth_stdrdDev.png');
